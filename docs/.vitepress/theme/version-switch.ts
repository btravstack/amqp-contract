...
// preserves the visitor's place: /amqp-contract/guide/worker-usage switches to
// /amqp-contract/beta/guide/worker-usage, same tab, search + hash included. A
// page missing from the target version lands on its 404 (which carries the nav
// to recover) — versions legitimately differ in page sets, and the 3.0 docs
// restructure means many pages moved.
//
...
const VERSION_ROOT = /^https:\/\/btravstack\.github\.io(\/amqp-contract\/(?:beta\/)?)$/;
...
export function setupVersionSwitch(): void {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      // Vite injects this build's own base (/amqp-contract/ or /amqp-contract/beta/).
