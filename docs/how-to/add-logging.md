...
| Level   | Covers                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `info`  | Successful consume; the retry decision (`Retrying message (requeue)` / `(republish)`, with `retryCount`); message published for retry (`delayMs` under `ttl-backoff`); `Sending to DLQ: <reason>` (non-retryable error, retry disabled, max retries exceeded, RPC requests are not retried) followed by the DLQ hand-off; discarding on a queue declared `onPoison: "drop"` |
| `warn`  | Consumer cancelled by the server; a settle the closing channel refused; **queue has neither a dead-letter exchange nor an `onPoison` declaration — message will be lost on nack**                                                                                                                                                                                           |
| `error` | Payload or header validation failed, or the body could not be decoded (`Failed to parse/validate message; sending to DLQ`); error processing message; retry publish failed (publish timeout, broker nack or channel closed); an RPC request with a disallowed `replyTo`; a defect (a handler or middleware that threw)                                                      |
...
The worker and `defineContract` answer "does this queue have a dead-letter exchange?" with the same predicate, so the two cannot disagree: **a DLX set through the raw `arguments` passthrough** (`arguments: { "x-dead-letter-exchange": "orders-dlx" }`) counts as configured everywhere. Such a queue passes the builder and logs the ordinary DLQ hand-off — it never trips this warning, and it is never described as discarded. What the worker still cannot see is a DLX applied as a **broker policy**: the contract carries no declaration, so it needs `onPoison: "drop"` to pass the builder and will log the discard line while the broker dead-letters correctly (see the `@amqp-contract/worker` changelog entry for the poison-log split).
