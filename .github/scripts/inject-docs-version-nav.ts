...
//   { "current": "v2.4.0", "items": [{ "text", "link", "target" }, ...] }
//
...
// (3.0.0), this script stops doing anything and can be deleted.
...
// NOTE the indentation: this repo's config nests the nav one level deeper than
// a plain `defineConfig` would, because it wraps in `withMermaid(...)`. The
// anchor is 8 spaces, not 6 — get this wrong and the script bails below rather
// than silently shipping a site with no version picker.
...
if (typeof window !== "undefined") {
  document.addEventListener(
    ...
    (event) => {
      ...
      const match = anchor
        ?.getAttribute("href")
        ?.match(/^https:\\/\\/btravstack\\.github\\.io(\\/amqp-contract\\/(?:beta\\/)?)$/);
      if (!anchor || !match?.[1]) {
        return;
      }
