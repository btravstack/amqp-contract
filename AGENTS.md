...
- Extractors go directly on the `AsyncResult` when terminal — `await client.close().getOrThrow()`, not `(await client.close()).getOrThrow()` (the extractor family is mirrored on `AsyncResult` and returns promises). `await` an `AsyncResult` down to a `Result` only when you need the Result itself: matchers (`expect(result).toBeErr()`), `isOk()`/`isErr()` branching, `.match(...)`, or passing it around.
