...
- Extractors go directly on the `AsyncResult` when terminal — `await client.close().get()`, not `(await client.close()).get()` (the extractor family is mirrored on `AsyncResult` and returns promises). Use `.get()` on `create()` / `close()` (their modeled channel is empty, `E = never` — a failure is a defect that `.get()` still panics on); reach for `.getOrThrow()` only on a still-fallible result like `publish()` / `call()`. `await` an `AsyncResult` down to a `Result` only when you need the Result itself: matchers (`expect(result).toBeErr()`), `isOk()`/`isErr()` branching, `.match(...)`, or passing it around.
...
- **Treating `await TypedAmqp(Client|Worker).create(...)` as a client/worker.** It returns `AsyncResult<Client, never>` (infrastructure failures are defects, not modeled errors); `await` gives you a `Result`. Unwrap with `.get()` (or pattern-match on the `defect` arm) before calling instance methods — `.getOrThrow()` won't compile on an empty error channel.
