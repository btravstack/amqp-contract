...
31. **A message published before any worker exists is retained, not dropped** (the client declares every queue its publishes route to, with the worker's exact arguments but none of its DLX / retry infrastructure; never an exclusive queue) — `tests/src/__tests__/publisher-topology.spec.ts` + `packages/core/src/setup.spec.ts` ("publisherTopology" describe).
