...
9. **RPC replies require `replyTo` + `correlationId`; undeclared error codes/invalid error data → DLQ, never a malformed reply** — `tests/src/__tests__/rpc.spec.ts` (undeclared-code/invalid-error-data timeout tests + "RPC DLQ routing" describe: DLQ delivery for undeclared codes and reply-less requests) A reply publish that fails at the core layer (write buffer full) routes the request to the DLQ through the modeled `NonRetryableError`, never the defensive defect fallback — `packages/worker/src/rpc-reply-failure.spec.ts`.
...
24. **A broker-side publish failure is a modeled `PublishError`, classified once in core** (timeout / nacked / buffer-full / channel-closed on the `E` channel; an unclassifiable rejection stays a defect) — `packages/core/src/publish-error.spec.ts` + `packages/client/src/rpc-timeout.spec.ts` ("a request the broker side refuses resolves Err(PublishError)…").
